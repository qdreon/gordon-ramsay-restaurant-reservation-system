...
async function searchAvailability(
  ...
  await partyInput.fill(String(partySize));

  await expect(dateInput).toHaveValue(date, { timeout: ASSERT_TIMEOUT });
  await expect(timeInput).toHaveValue(time, { timeout: ASSERT_TIMEOUT });
  await expect(partyInput).toHaveValue(String(partySize), {
    timeout: ASSERT_TIMEOUT,
  });

  // Submit the availability search form and wait for loading to finish.
  await page.click('button[type="submit"]');
  await expect(
