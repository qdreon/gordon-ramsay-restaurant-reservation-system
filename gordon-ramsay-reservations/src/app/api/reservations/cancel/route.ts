  ...
  try {
    ...
    if (timeUntilStart < TWO_HOURS_IN_MS) {
      return NextResponse.json(
        {
          error:
            "Cancellation is only allowed at least 2 hours before reservation time.",
        },
        { status: 400 },
      );
    }
    ...
    try {
      ...
      const { data: offeredWaitlist, error: waitlistError } =
        await adminSupabase
          .from("waitlist")
          .select(
            "id, customer_id, desired_date, desired_time, party_size, position",
          )
      ...
      if (!waitlistError && offeredWaitlist) {
        const { data: waitlistCustomer, error: customerLookupError } =
          await adminSupabase
            .from("customers")
        ...
        if (!customerLookupError && waitlistCustomer) {
          ...
          if (userRecord?.email) {
            await sendWaitlistInvite({
              ...
              requestedTime: new Date(offeredWaitlist.desired_time)
                .toISOString()
                .slice(11, 16),
              restaurantName:
              ...
              confirmationURL: `${process.env.NEXT_PUBLIC_APP_URL ?? "http://localhost:3000"}/customer/dashboard?waitlist=offered`,
            });
          }
        }
      }
      ...
      console.error(
        "[Cancellation Notification] Failed to send waitlist invitation:",
        emailError,
      );
    }
    ...
    const message =
      error instanceof Error
        ? error.message
        : "Unexpected error while cancelling reservation.";
