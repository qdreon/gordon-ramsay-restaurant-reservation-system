...
export interface AdminReservationRangeRow {
  id: string;
  customer_id: string;
...
export async function createPendingReservationLock(
  ...
  if (error) {
    throw new Error(
      `[Reservation Service] Failed to lock reservation: ${error.message}`,
    );
  }
  ...
  if (!firstRow) {
    throw new Error(
      "[Reservation Service] Reservation lock RPC returned no result.",
    );
  }
...
export async function releaseExpiredPendingReservations(): Promise<number> {
  ...
  if (error) {
    throw new Error(
      `[Reservation Service] Failed to release expired locks: ${error.message}`,
    );
  }
...
export async function getReservationsByDateRange(
  startDate: string,
  endDate: string,
): Promise<AdminReservationRangeRow[]> {
  const supabase = createServiceSupabaseClient();

  const { data, error } = await supabase
    .from("reservations")
    .select(
      "id, customer_id, reservation_date, start_time, end_time, party_size, status, special_requests, locked_until",
    )
  ...
  if (error) {
    throw new Error(
      `[Reservation Service] Failed to fetch reservations: ${error.message}`,
    );
  }

  return (data ?? []) as AdminReservationRangeRow[];
}
...
export async function createBlockedDate(
  ...
  if (error) {
    throw new Error(
      `[Reservation Service] Failed to create blocked date: ${error.message}`,
    );
  }
...
export async function getBlockedDatesForMonth(
  ...
  if (error) {
    throw new Error(
      `[Reservation Service] Failed to fetch blocked dates: ${error.message}`,
    );
  }

  return ((data ?? []) as BlockedDateRow[]).map((row) => row.blocked_date);
}
