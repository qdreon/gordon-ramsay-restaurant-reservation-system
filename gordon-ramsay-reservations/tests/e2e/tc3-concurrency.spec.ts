 ...
 *   - Measure the wall-clock time from fire to resolution (PR-2: < 12s tolerance in e2e).
 *
...
function buildReservationRange(reservationDate: string, reservationTime: string) {
...
async function getCustomerId(email: string): Promise<string> {
  ...
  if (userError || !userRow) {
    throw new Error(`Unable to resolve user row for ${email}: ${userError?.message ?? "not found"}`);
  }
...
test.describe("TC-3.2 Concurrency Lock Resolution (FR-3, PR-2) [QDR-49]", () => {
  test("two simultaneous bookings: one must succeed, one must receive a lock conflict error", async () => {
    ...
    expect(
      elapsedMs,
      `Lock conflict must resolve within 12 000ms total round-trip (PR-2 DB lock = 1s). Got ${elapsedMs}ms`,
    ).toBeLessThan(12000);
    ...
    expect(conflictAttempt, "One booking request must fail with a lock conflict").toBeDefined();
    expect(conflictAttempt?.payload.error ?? "").toMatch(/reserved|lock|55P03/i);
  });
});
